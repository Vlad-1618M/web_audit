...
| **Mac app — public** | Site owners (no Docker) | `WebAudit-*-macOS.dmg` | Drag to Applications | `macos-v*` (planned) | e.g. `macos-v0.1.0` after notarization |
...
| Mac app marketing version | `macos/WebAuditMac/VERSION` | `0.1.0-alpha` |
...
### Not tagged yet (in progress / planned)

| Planned tag | Artifact | Status |
|-------------|----------|--------|
| `macos-v0.1.0-alpha` | Public DMG `WebAudit-0.1.0-alpha-macOS.dmg` | Workflow ready (`.github/workflows/release-macos.yml`); gets tagged post QA approvals after local tests |
...
|--------------|--------------|---------------|-------------|
| `0.1.0-alpha` | `WebAudit-0.1.0-alpha-macOS.dmg` | bundled (public) | `packaging/public-installer/INSTALL.txt` |
| `0.1.0-alpha` | `WebAudit-0.1.0-alpha-macOS-dev.dmg` | external Docker/venv | `packaging/dev-external-engine/INSTALL.txt` |
...
**Git tag for Mac releases:** `macos-v<VERSION>` (e.g. `macos-v0.1.0-alpha`) — separate from engine `v2.x` tags so Docker users are not confused.
...
3. **Mac app** — stay on `0.x` marketing version while engine is `2.x`? (yes for now: app `0.1.0-alpha`, engine `2.1.0b4`)
4. **GHCR** — publish `:stable` alias when engine hits stable, or only semver + `latest`?
5. **Bundled Mac DMG** — record engine semver in release notes / `INSTALL.txt` each build?
...
| [v2_python_core/docs/release_flow_t3_stage_6.md](v2_python_core/docs/release_flow_t3_stage_6.md) | Example tag flow (archive after use) |
...
*Last updated: 2026-06-05 — fill in `macos-v0.1.0-alpha` row when tagged.*<br>
*-Vlad.M*
