...
def _base_run(**artifact_inventory) -> AuditRun:
    inventory = {
        "html": {"pages_scanned": 1, "inventory": {"link_count": 5, "site_links": []}},
        **artifact_inventory,
    }
    return AuditRun(
        meta=AuditMeta(
        ...
        findings=[],
        artifacts=AuditArtifacts(inventory=inventory),
    )


def test_api_section_not_used():
    section = build_api_section(
        {"inventory": {"html": {"inventory": {"link_count": 5}}}},
        target_url="https://example.com",
    )
    ...
    section = build_api_section(
        {
            "inventory": {
                "api": {
                    "graphql": [
                        {"path": "/graphql", "status_code": 404, "introspection_enabled": False},
                    ],
                    "openapi": [
                        {"path": "/openapi.json", "status_code": 404, "openapi_detected": False},
                    ],
                }
            }
        },
...
def test_render_technical_shows_api_ran():
    run = _base_run(
        api={
            "graphql": [
                {"path": "/graphql", "status_code": 200, "introspection_enabled": False},
            ],
            "openapi": [
                {
                    "path": "/openapi.json",
