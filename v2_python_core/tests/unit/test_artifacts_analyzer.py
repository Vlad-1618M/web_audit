...
def test_security_txt_stale_expires():
    """Ensures Security TXT Stale Expires."""
    probe = ArtifactsProbeResult(target_url='https://example.com', security_txt=SecurityTxtArtifact(present=True, body='Contact: mailto:sec@example.com\nExpires: 2020-01-01\n'))
...
def test_robots_open_path_conflict():
    """Ensures Robots Open Path Conflict."""
    probe = ArtifactsProbeResult(target_url='https://example.com', robots=RobotsArtifact(status_code=200, raw='Disallow: /backup/\n'))
