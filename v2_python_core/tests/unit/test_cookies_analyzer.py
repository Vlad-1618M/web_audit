...
def test_insecure_session_cookie():
    probe = CookiesProbeResult(
        target_url="https://example.com",
        source_url="https://example.com/login/",
        cookies=[CookieEntry(name="sessionid", raw="sessionid=abc; Path=/")],
    )
...
def test_secure_cookie_ok():
    probe = CookiesProbeResult(
        ...
        cookies=[
            CookieEntry(
                name="sid",
                raw="sid=abc; Path=/; HttpOnly; Secure; SameSite=Lax",
            )
        ],
    )
    findings = analyze_cookies(probe, CookiesCollectorSettings(), framework="auto")
...
def test_django_csrf_expected():
    probe = CookiesProbeResult(
        target_url="https://example.com",
        cookies=[CookieEntry(name="csrftoken", raw="csrftoken=abc; Path=/")],
    )
