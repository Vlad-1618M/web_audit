...
def test_cors_wildcard_high():
    """Ensures Cors Wildcard High."""
    probe = CorsProbeResult(target_url='https://example.com', origin_sent='https://evil.example.com', probes=[CorsProbeEntry(path='/api/', status_code=200, acao='*')])
...
def test_cors_reflected_critical():
    """Ensures Cors Reflected Critical."""
    probe = CorsProbeResult(target_url='https://example.com', origin_sent='https://evil.example.com', probes=[CorsProbeEntry(path='/api/v1/', status_code=200, acao='https://evil.example.com')])
    findings = analyze_cors(probe, CorsCollectorSettings())
    assert findings[0].status == 'REFLECTED'
    assert findings[0].severity == Severity.CRITICAL

def test_cors_none_on_paths():
    """Ensures Cors None On Paths."""
    probe = CorsProbeResult(target_url='https://example.com', origin_sent='https://evil.example.com', probes=[CorsProbeEntry(path='/api/', status_code=404, acao=None)])
    findings = analyze_cors(probe, CorsCollectorSettings())
