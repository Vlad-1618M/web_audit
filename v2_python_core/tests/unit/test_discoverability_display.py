...
def test_discoverability_empty_state():
    """Ensures Discoverability Empty State."""
...
def test_discoverability_lists_seo_findings():
    """Ensures Discoverability Lists SEO Findings."""
    findings = [Finding.from_check(category='SEO_SURFACE', item='Meta description', status='MISSING', severity=Severity.INFO, detail='No meta description on homepage', class_=FindingClass.INFO, scored=False), Finding.from_check(category='SEO_SURFACE', item='Meta robots', status='NOINDEX', severity=Severity.INFO, detail='Homepage has noindex', class_=FindingClass.VERIFY, scored=False)]
