    ...
    def get(self, url, headers=None):
        ...
        if "packagist.org" in url and "laravel/framework" in url:
            return _FakeResponse(
                200,
                {"packages": {"laravel/framework": [{"version": self.versions.get("laravel/framework", "11.0")}]}},
            )
...
def test_django_stale_package_action():
    profile = FrameworkProfile(
        framework="django",
        compare=ProfileCompareSettings(pypi_api=True),
        watchlist=[WatchlistEntry(package="django", tier="critical")],
    )
    probe = ExtensionsProbeResult(
        target_url="https://example.com",
        framework="django",
        unit="package",
        extensions=[ObservedExtension(name="django", version="4.2.0", unit="package")],
    )
