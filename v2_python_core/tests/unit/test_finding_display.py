...
from webaudit.render.finding_display import (
    enrich_findings_table,
    finding_status_tone,
...
def test_verify_detail_highlights_unexpected_open_path():
    finding = Finding.from_check(
        ...
        status="OPEN",
        severity=Severity.MEDIUM,
        detail="Unexpected open path — raw 200 → final 200; verify public access is intended",
        class_=FindingClass.VERIFY,
        scored=False,
    )
...
def test_action_sensitive_path_detail_highlight():
    finding = Finding.from_check(
        ...
        detail="Sensitive path reachable — raw 200 → final 200 (OPEN)",
