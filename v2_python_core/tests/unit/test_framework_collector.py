...
def test_wordpress_body_signals_win():
    scores = _score_framework(
        body='<link href="/wp-content/themes/twentytwenty/style.css">',
        cookie_text="wordpress_logged_in=1",
...
def test_django_admin_and_csrf_signals():
    scores = _score_framework(
        ...
        cookie_text="csrftoken=abc; sessionid=xyz",
        headers={},
        admin_body="Django administration id_username",
    )
