...
def test_js_skipped_without_playwright():
    """Ensures Js Skipped Without Playwright."""
    with patch.dict('sys.modules', {'playwright': None, 'playwright.sync_api': None}):
        probe = collect_js_render('https://example.com', wait_seconds=1.0, user_agent='WebAudit/test', timeout_seconds=5)
...
def test_js_render_inventory_counts():
    """Ensures Js Render Inventory Counts."""
    ...
    with patch.dict('sys.modules', {'playwright': MagicMock(), 'playwright.sync_api': fake_sync_api}):
        probe = collect_js_render('https://example.com', wait_seconds=0.0, user_agent='WebAudit/test', timeout_seconds=5)
