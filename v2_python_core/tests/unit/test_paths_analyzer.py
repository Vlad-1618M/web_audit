...
def _probe(*entries: tuple[str, int, int, str]) -> PathsProbeResult:
    return PathsProbeResult(target_url='https://example.com', probes=[PathProbeEntry(path=p, raw_status=raw, final_status=final, note=note) for p, raw, final, note in entries])

def test_sensitive_open_is_scored_action():
    """Ensures Sensitive Open Is Scored Action."""
    findings = analyze_paths(_probe(('/.env', 200, 200, 'OPEN')), framework='auto', paths_settings=PathsSettings())
...
def test_sensitive_server_error_is_leaking():
    """Ensures Sensitive Server Error Is Leaking."""
    findings = analyze_paths(_probe(('/wp-config.php', 500, 500, 'SERVER_ERROR')), framework='auto', paths_settings=PathsSettings())
...
def test_not_found_emits_no_finding():
    """Ensures Not Found Emits No Finding."""
    findings = analyze_paths(_probe(('/.env', 404, 404, 'NOT_FOUND')), framework='auto', paths_settings=PathsSettings())
    assert findings == []

def test_robots_open_is_expected():
    """Ensures Robots Open Is Expected."""
    findings = analyze_paths(_probe(('/robots.txt', 200, 200, 'OPEN')), framework='auto', paths_settings=PathsSettings())
    assert len(findings) == 1
    assert findings[0].class_ == FindingClass.EXPECTED
    assert findings[0].scored is False

def test_unexpected_open_is_verify_only():
    """Ensures Unexpected Open Is Verify Only."""
    findings = analyze_paths(_probe(('/readme.html', 200, 200, 'OPEN')), framework='auto', paths_settings=PathsSettings())
    assert len(findings) == 1
