...
from webaudit.collectors.paths import build_probe_list, classify_final_status, collect_paths
from webaudit.config.settings import PathsSettings

def test_classify_final_status():
    """Ensures Classify Final Status."""
...
def test_build_probe_list_respects_limit_and_extra():
    """Ensures Build Probe List Respects Limit And Extra."""
    settings = PathsSettings(sensitive_builtin=False, extra_paths=['/.env', '/custom'], max_probe_urls=2)
...
def test_collect_paths_probes_with_mock_client():
    """Ensures Collect Paths Probes With Mock Client."""
    ...
    result = collect_paths('https://example.com', framework='auto', paths_settings=settings, user_agent='test', timeout_seconds=5, client=FakeClient())
    assert len(result.probes) == 1
    assert result.probes[0].path == '/.env'
    assert result.probes[0].note == 'NOT_FOUND'

def test_collect_paths_on_probe_callbacks():
    """Ensures Collect Paths On Probe Callbacks."""
    ...
    class FakeClient:
        ...
        def close(self):
            pass
    settings = PathsSettings(sensitive_builtin=False, extra_paths=['/a', '/b'])
    started: list[int] = []
    done: list[tuple[str, int, int]] = []
    collect_paths('https://example.com', framework='auto', paths_settings=settings, user_agent='test', timeout_seconds=5, client=FakeClient(), on_probe_start=started.append, on_probe=lambda entry, index, total: done.append((entry.path, index, total)))
    assert started == [2]
