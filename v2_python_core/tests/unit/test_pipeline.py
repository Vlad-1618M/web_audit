...
def test_pipeline_merges_headers_and_dns(mock_resolver_cls, httpx_mock):
    """Ensures Pipeline Merges Headers And DNS."""
    httpx_mock.add_response(method='HEAD', url='https://example.com', headers={'Strict-Transport-Security': 'max-age=31536000'})
    resolver = MagicMock()
...
def test_pipeline_paths_enabled(httpx_mock):
    """Ensures Pipeline Paths Enabled."""
    httpx_mock.add_response(method='HEAD', url='https://example.com', headers={})
...
def test_pipeline_tls_enabled(mock_collect_tls, httpx_mock):
    """Ensures Pipeline TLS Enabled."""
    from webaudit.collectors.tls import TlsCertificateInfo, TlsProbeResult, TlsVersionResult
    httpx_mock.add_response(method='HEAD', url='https://example.com', headers={})
    mock_collect_tls.return_value = TlsProbeResult(target_url='https://example.com', host='example.com', port=443, versions=[TlsVersionResult(version='1.2', supported=True, negotiated='TLSv1.2')], certificate=TlsCertificateInfo(subject='CN=example.com', issuer='CN=CA', not_after='2099-01-01T00:00:00+00:00', days_left=9000, chain_length=2))
    settings = Settings(target=TargetSettings(url='https://example.com'))
    settings.collectors.dns.enabled = False
    settings.collectors.cookies.enabled = False
    settings.collectors.artifacts.enabled = False
