...
def test_hsts_short_max_age_is_scored():
    """Ensures HSTS Short Max Age Is Scored."""
...
def test_hsts_missing_include_subdomains_info():
    """Ensures HSTS Missing Include Subdomains Info."""
...
def test_csp_unsafe_inline_and_wildcard():
    """Ensures CSP Unsafe Inline And Wildcard."""
    headers = {'content-security-policy': "default-src *; script-src 'unsafe-inline' https://cdn.example.com"}
    findings, artifact = analyze_policy(headers, PolicySettings())
...
def test_csp_clean_emits_review_ok():
    """Ensures CSP Clean Emits Review Ok."""
...
def test_no_headers_emits_nothing():
    """Ensures No Headers Emits Nothing."""
