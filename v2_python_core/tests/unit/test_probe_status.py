...
from webaudit.render.probe_status import build_probe_status_row, classify_probe_outcome, http_status_tone

def test_sensitive_not_found_is_good():
    """Ensures Sensitive Not Found Is Good."""
    tone, hint = classify_probe_outcome('/.env', 'NOT_FOUND')
    assert tone == 'good'
    assert 'Hidden' in hint or 'good' in hint.lower()

def test_sensitive_open_is_bad():
    """Ensures Sensitive Open Is Bad."""
...
def test_build_probe_status_row_includes_tones():
    """Ensures Build Probe Status Row Includes Tones."""
    row = build_probe_status_row({'path': '/.env', 'final_status': 404, 'note': 'NOT_FOUND'}, framework='wordpress')
...
def test_exposed_200_is_bad_outcome_but_green_http_code():
    """Ensures Exposed 200 Is Bad Outcome But Green HTTP Code."""
    row = build_probe_status_row({'path': '/.git/HEAD', 'final_status': 200, 'note': 'OPEN'})
