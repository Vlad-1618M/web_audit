...
def test_post_unprotected_scored():
    """Ensures Post Unprotected Scored."""
    probe = RateLimitProbeResult(target_url='https://example.com', login_path='/login/', post_kind='form', get=RateLimitGetResult(attempts=6), post=RateLimitPostResult(attempts=15))
...
def test_post_protected_ok():
    """Ensures Post Protected Ok."""
    probe = RateLimitProbeResult(target_url='https://example.com', login_path='/login/', post_kind='form', get=RateLimitGetResult(attempts=6, limited_at=3, limit_status=429), post=RateLimitPostResult(attempts=15, limited_at=5, limit_status=429))
    findings = analyze_rate_limit(probe, RateLimitCollectorSettings())
