...
def test_metric_chips_count_severities():
    findings = [
        Finding.from_check(category="HEADERS", item="CSP", status="MISSING", severity=Severity.HIGH),
        Finding.from_check(
            category="PATHS", item=".env", status="OPEN", severity=Severity.CRITICAL, class_=FindingClass.ACTION
        ),
        Finding.from_check(
