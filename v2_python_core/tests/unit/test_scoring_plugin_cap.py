...
def _stale_plugin(slug: str) -> Finding:
    return Finding.from_check(category='PLUGIN', item=slug, status='STALE', severity=Severity.HIGH, class_=FindingClass.ACTION, scored=True)

def test_plugin_worst_wins_caps_at_thirty_with_five_stale_plugins():
    """Ensures Plugin Worst Wins Caps At Thirty With Five Stale Plugins."""
...
def test_five_stale_plugins_without_worst_wins_caps_at_seventy():
    """Ensures Five Stale Plugins Without Worst Wins Caps At Seventy."""
    findings = [_stale_plugin(f'plugin-{i}') for i in range(5)]
...
def test_seo_surface_never_affects_hygiene_by_default():
    """Ensures SEO Surface Never Affects Hygiene By Default."""
    findings = [Finding(category='SEO_SURFACE', item='meta robots', status='NOINDEX', severity=Severity.INFO, class_=FindingClass.VERIFY, scored=True)]
