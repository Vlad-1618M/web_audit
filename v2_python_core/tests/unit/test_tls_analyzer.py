...
def _probe(**kwargs) -> TlsProbeResult:
    defaults = {'target_url': 'https://example.com', 'host': 'example.com', 'port': 443}
    defaults.update(kwargs)
    return TlsProbeResult(**defaults)

def test_tls_11_accepted_is_scored_action():
    """Ensures TLS 11 Accepted Is Scored Action."""
    probe = _probe(versions=[TlsVersionResult(version='1.0', supported=False), TlsVersionResult(version='1.1', supported=True, negotiated='TLSv1.1'), TlsVersionResult(version='1.2', supported=True, negotiated='TLSv1.2'), TlsVersionResult(version='1.3', supported=True, negotiated='TLSv1.3')])
...
def test_expired_cert_is_critical():
    """Ensures Expired Cert Is Critical."""
    probe = _probe(versions=[], certificate=TlsCertificateInfo(subject='CN=example.com', issuer='CN=Test CA', not_after='2020-01-01T00:00:00+00:00', days_left=-100, chain_length=2))
    findings = analyze_tls(probe, TlsCollectorSettings())
...
def test_http_skipped_probe_emits_nothing():
    """Ensures HTTP Skipped Probe Emits Nothing."""
...
def test_chain_info_when_enabled():
    """Ensures Chain Info When Enabled."""
    probe = _probe(versions=[], certificate=TlsCertificateInfo(subject='CN=example.com', issuer='CN=Test CA', not_after='2099-01-01T00:00:00+00:00', days_left=9000, chain_length=3, san=['example.com', 'www.example.com']))
