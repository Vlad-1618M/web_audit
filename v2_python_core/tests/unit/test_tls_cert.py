...
from webaudit.collectors.tls_cert import build_cert_issues, format_days_left, issuer_display_name, validate_hostname

def test_issuer_display_lets_encrypt():
    """Ensures Issuer Display Lets Encrypt."""
    name = issuer_display_name(issuer="CN=R3,O=Let's Encrypt,C=US", issuer_org="Let's Encrypt", issuer_cn='R3')
    assert "Let's Encrypt" in name

def test_issuer_display_hashicorp():
    """Ensures Issuer Display Hashicorp."""
    name = issuer_display_name(issuer='CN=HashiCorp Vault,O=HashiCorp', issuer_org='HashiCorp', issuer_cn='HashiCorp Vault')
    assert 'HashiCorp' in name

def test_validate_hostname_wildcard_san():
    """Ensures Validate Hostname Wildcard San."""
    ok, note = validate_hostname('www.example.com', subject='CN=example.com', san=['*.example.com', 'example.com'])
    assert ok is True
...
def test_build_cert_issues_expiring_and_tls():
    ...
    cert = {'days_left': 12, 'not_after': '2026-06-15T00:00:00+00:00', 'hostname_match': True}
...
def test_format_days_left_expired():
    """Ensures Format Days Left Expired."""
