...
def test_stale_free_plugin_is_scored_action():
    profile = FrameworkProfile(
        ...
        watchlist=[WatchlistEntry(slug="akismet", tier="medium")],
    )
    probe = WpPluginsProbeResult(
        target_url="https://example.com",
        plugins=[ObservedPlugin(slug="akismet", version_html="5.0")],
    )
    client = _FakeClient({"akismet": "5.3"})
    findings = analyze_wp_plugins(
        probe,
        profile,
        user_agent="test",
        timeout_seconds=5,
...
def test_premium_plugin_is_verify_not_scored():
    profile = FrameworkProfile(
        framework="wordpress",
        watchlist=[WatchlistEntry(slug="wp-rocket", tier="high", premium=True, compare_latest=False)],
    )
    probe = WpPluginsProbeResult(
        target_url="https://example.com",
        plugins=[ObservedPlugin(slug="wp-rocket", version_html="3.15")],
    )
    findings = analyze_wp_plugins(
        probe,
        profile,
