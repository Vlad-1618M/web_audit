...
def test_extract_plugins_from_html_with_versions():
    """Ensures Extract Plugins From HTML With Versions."""
...
def test_readme_parser_rejects_html_soft_404():
    """Ensures Readme Parser Rejects HTML Soft 404."""
...
def test_collect_wp_plugins_observed_only(monkeypatch):
    """Ensures Collect WordPress Plugins Observed Only."""
    from webaudit.collectors import wp_plugins as mod
    profile = FrameworkProfile(framework='wordpress', probe=ProfileProbeSettings(mode='observed_only', readme_fetch='observed_only'))
